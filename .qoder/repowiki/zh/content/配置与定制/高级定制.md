...
## 更新摘要
**变更内容**
- 移除了复杂的高级定制配置系统，简化为基本的国际化和界面定制能力
- 保留了 USER_PROMPT_PRESETS 预设场景的基础使用
- 简化了高还原度模式配置选项
- 保持了自定义提示词模板的基本功能
- 维持了 UI_STRINGS 与 SETTINGS_I18N 的国际化机制
...
- USER_PROMPT_PRESETS 预设场景的使用与自定义方法，覆盖 general、photo、cg、design、ui、assets3d、product 等模板及其适用场景与配置要点
- 高还原度模式配置选项的启用与使用，支持更精确的图像还原提示词生成
- 自定义提示词模板的创建、命名、保存与删除流程
- UI_STRINGS 与 SETTINGS_I18N 的国际化配置机制及界面文本调整方法
- 面向用户的定制技巧与最佳实践，帮助按工作流优化使用体验

**更新** 本版本已简化为基本的定制能力，移除了复杂的高级配置系统
...
**更新** 移除了 anime、architecture、food、portrait 等专业领域模板，保留了基础场景
...
- 适用场景：需要极高精度图像还原的专业场景，如产品摄影、艺术创作、设计复原等

**更新** 保留了高还原度模式配置选项，但简化了相关功能

**章节来源**
- [config.js:21](file://config.js#L21)
- [config.js:145](file://config.js#L145)
- [config.js:195](file://config.js#L195)
- [options.html:714](file://options.html#L714)
...
- 高还原度模式应用
  - 在需要极高精度还原的场景下启用高还原度模式
  - 结合专业的摄影、设计、艺术等领域模板获得更精确的结果
- 语言与界面一致性
...
## 结论
通过 USER_PROMPT_PRESETS 的场景化模板、高还原度模式配置选项、自定义模板的灵活扩展、UI_STRINGS 与 SETTINGS_I18N 的双语支持，以及后台与内容脚本的协同，Img2Prompt 提供了简洁实用的定制能力。相比之前的复杂配置系统，本版本专注于基础的国际化和界面定制，移除了高级选项但仍保留了核心功能。结合最佳实践与故障排查方法，用户可以按自身工作流高效地生成与复用高质量提示词，并在多语言环境下保持一致的使用体验。

## 附录
- 术语
  - 预设：内置的 user prompt 模板
  - 自定义模板：用户创建并保存的 user prompt 模板
  - 高还原度模式：启用更详细分析提示词的配置选项
  - 双语：中文与英文两种界面语言
- 相关文件路径
  - [config.js](file://config.js)
  - [background.js](file://background.js)
