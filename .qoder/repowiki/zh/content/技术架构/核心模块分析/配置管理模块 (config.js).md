...
- **向后兼容**：支持配置项的动态更新和迁移
- **模块化设计**：清晰的配置分类和组织结构
- **严格验证**：强制要求严格的JSON结构，确保配置完整性

**更新** 重构了系统提示和用户提示，强制要求严格的JSON结构，新增负向提示词字段和参数对象定义，增强了配置系统的完整性和准确性。
...
- [background.js:1-1142](file://background.js#L1-L1142)
- [content.js:1-1748](file://content.js#L1-L1748)
...
默认设置是配置系统的核心基础，包含了扩展运行所需的所有必要配置项。**更新** 现在强制要求严格的JSON结构：
...
| systemPrompt | string | 严格的JSON结构提示 | 系统级提示词，强制JSON格式 |
| userPrompt | string | 结构化分析提示 | 用户级提示词，强制JSON格式 |
| temperature | number | 1 | 生成温度参数 |

**更新** 系统提示词和用户提示词现在强制要求严格的JSON结构，包含负向提示词和参数对象。

### 严格JSON结构要求

系统现在要求所有提示词输出必须符合严格的JSON结构：
...
系统提供了多种场景化的提示词预设，每种预设针对特定的图像分析需求。**更新** 所有预设现在都支持严格的JSON结构：
...
**更新** 新增四个专业领域模板：anime、architecture、food、portrait，每个都包含详细的分析维度和专业术语，并支持严格的JSON结构。

### UI 字符串本地化 (UI_STRINGS)

UI 字符串系统支持中英文双语界面，包含完整的界面文本资源。**更新** 新增了JSON解析相关的错误消息：
...
配置管理模块采用分层架构设计，确保配置的统一管理和高效访问。**更新** 增加了严格的JSON验证机制：
...
配置验证系统确保所有配置项的有效性和一致性。**更新** 现在包含严格的JSON结构验证：
...
Config->>Config : 检查负向提示词完整性
Config->>Config : 验证参数对象格式
...
系统实现了多层次的配置更新机制。**更新** 现在包含严格的JSON验证：

1. **自动保存延迟机制**：防抖处理，避免频繁写入
2. **实时变更监听**：即时响应配置变化
3. **跨组件同步**：确保所有扩展组件保持配置一致
4. **回滚机制**：支持配置恢复到默认值
5. **严格JSON验证**：确保配置项符合严格的JSON结构要求

**更新** 高还原度模式的动态更新策略已集成到现有的配置更新机制中，包含严格的JSON验证。
...
配置系统采用层次化的组织结构，每个配置类别都有明确的职责分工。**更新** 现在包含严格的JSON结构要求：
...
模型参数配置支持灵活的参数调整。**更新** 现在包含严格的JSON结构要求：
...
界面语言系统支持动态切换和持久化存储。**更新** 现在包含JSON解析错误的本地化支持：
...
行为偏好配置管理用户交互设置。**更新** 现在包含高还原度模式的专业配置：
...
**更新** 新增高还原度模式配置项，提供专业的图像还原分析能力，支持严格的JSON结构。

### 专业领域模板配置

系统新增的四个专业领域模板提供了深度的分析能力。**更新** 所有模板现在都支持严格的JSON结构：
...
- **色彩搭配**：动漫常用色系和主题色调
- **构图技巧**：三分法、动态角度、特写镜头
- **负向提示词**：低分辨率、坏解剖、坏手、缺失手指、额外肢体、绘制糟糕的脸、变异、变形、模糊、坏解剖、毁容
- **参数对象**：包含质量标签和风格参数
...
配置管理模块与扩展其他组件的依赖关系。**更新** 现在包含JSON验证相关的依赖：
...
subgraph "JSON验证依赖"
SanitizeJSON[JSON清理函数]
ValidateJSON[JSON验证函数]
NormalizeResult[结果规范化]
end
Config --> OptionsJS
Config --> BackgroundJS
Config --> ContentJS
...
Config --> EnMessages
Config --> ZhMessages
SanitizeJSON --> ValidateJSON
ValidateJSON --> NormalizeResult
NormalizeResult --> BackgroundJS
```
...
配置验证系统确保数据的完整性和有效性。**更新** 现在包含严格的JSON结构验证：
...
RecreateMode --> |禁用| StrictJSON{严格JSON验证}
AdvancedCheck --> StrictJSON
StrictJSON --> |失败| Error4[错误: JSON格式不正确]
StrictJSON --> |成功| NegativePrompt{负向提示词验证}
NegativePrompt --> |失败| Error5[错误: 负向提示词缺失]
NegativePrompt --> |成功| Parameters{参数对象验证}
Parameters --> |失败| Error6[错误: 参数对象格式错误]
Parameters --> |成功| Success[验证通过]
...
配置管理模块在设计时充分考虑了性能优化。**更新** 现在包含JSON验证的性能考虑：

### 内存管理
- **懒加载策略**：配置项按需加载，避免不必要的内存占用
- **缓存机制**：常用配置项缓存在内存中，减少存储访问开销
- **垃圾回收**：及时清理不再使用的配置引用
- **JSON解析缓存**：缓存JSON解析结果，避免重复解析

### 网络优化
- **批量存储**：配置更新采用批量写入，减少存储 API 调用次数
- **增量更新**：只更新发生变化的配置项
- **去重处理**：避免重复的配置更新操作
- **JSON验证优化**：使用高效的JSON验证算法

### 用户体验优化
- **防抖处理**：配置变更采用防抖机制，避免频繁的界面更新
- **异步处理**：配置保存操作异步执行，不影响用户操作
- **渐进式加载**：配置项按需加载，提升初始启动速度
- **JSON验证反馈**：提供即时的JSON格式验证反馈

**更新** 高还原度模式的引入增加了配置项的复杂性，但通过合理的缓存和懒加载策略确保性能不受影响。严格的JSON验证机制也经过优化，确保不会影响用户体验。
...
| JSON格式错误 | 模型返回非JSON格式 | 调整 System Prompt 确保输出严格JSON |
| 负向提示词缺失 | 负面提示词为空 | 检查 System Prompt 中的负向提示词字段 |
| 参数对象错误 | 参数格式不正确 | 验证 parameters 对象的结构和类型 |

**更新** 新增高还原度模式和严格JSON结构相关的故障排除指导。

### 配置迁移策略

系统支持配置的平滑迁移和版本兼容。**更新** 现在包含JSON结构迁移：
...
RecreateMode --> |已有| StrictJSON{严格JSON验证}
InitMode --> StrictJSON
...
系统通过以下机制确保版本兼容性。**更新** 现在包含JSON结构兼容性：
...
**更新** 重构的系统提示和用户提示，强制要求严格的JSON结构，新增负向提示词字段和参数对象定义，显著增强了系统的专业性和准确性。新增的严格JSON验证机制确保了配置的完整性和可靠性。
...
8. **严格JSON格式**：确保System Prompt和User Prompt输出严格的JSON结构
9. **负向提示词管理**：为每个专业模板提供针对性的负向提示词
10. **参数对象配置**：合理设置技术参数以获得最佳生成效果

该配置管理模块为 ImgPrompt 扩展的成功运行奠定了坚实的基础，其设计理念和实现方式值得其他浏览器扩展项目借鉴和学习。新增的专业功能和严格的JSON验证机制进一步提升了系统的实用性和专业性，为用户提供了更加精准和专业的图像分析体验。