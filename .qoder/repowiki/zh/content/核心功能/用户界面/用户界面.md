...
- 完善悬浮入口和拖拽移动交互
- 增强历史记录管理和导出功能
- **新增多语言支持增强功能**：设置面板语言切换按钮、主面板双语言显示模式、完整的国际化资源
...
### 设置面板语言切换功能
- **语言切换按钮**：在"面板语言"区域提供中文/英文切换按钮
- **即时应用**：切换后立即更新设置面板的所有文案与占位符
- **状态保持**：切换语言时保持当前设置不变，仅改变显示语言
- **视觉反馈**：激活的语言按钮具有高亮样式，便于识别当前状态

### 主面板双语言显示模式
- **语言切换按钮**：主面板顶部提供"中文"/"English"/"JSON"三个按钮
- **双语言显示**：支持中文和英文两种语言模式的提示词显示
- **JSON模式**：提供结构化JSON显示模式，便于高级用户编辑
- **状态同步**：语言切换时同步更新按钮状态和内容显示
- **偏好记忆**：切换语言后会持久化用户的语言偏好设置

### 国际化资源完善
- **扩展级国际化**：通过 `_locales/zh_CN` 和 `_locales/en` 目录提供完整的本地化资源
- **消息键系统**：使用 `__MSG_extName__` 和 `__MSG_extDescription__` 等消息键
- **动态加载**：根据浏览器语言自动选择合适的本地化资源
- **清单集成**：在 manifest.json 中正确配置默认语言和本地化消息

### 语言切换实现机制
- **配置共享**：通过 `ImgPromptConfig.UI_STRINGS` 提供统一的多语言文案
- **运行时切换**：支持在不重启扩展的情况下动态切换语言
- **状态同步**：语言切换时同步更新所有相关组件的显示内容
- **错误处理**：提供降级机制，当特定语言缺失时自动回退到默认语言

```mermaid
flowchart TD
UserClick["用户点击语言切换按钮"] --> CheckLang["检查当前语言状态"]
CheckLang --> UpdateState["更新语言状态和偏好设置"]
UpdateState --> ApplyDict["应用对应的翻译字典"]
ApplyDict --> UpdateUI["更新界面元素显示"]
UpdateUI --> PersistPref["持久化语言偏好"]
PersistPref --> NotifyOther["通知其他组件更新"]
NotifyOther --> Complete["切换完成"]
```

**图表来源**
- [options.js:687-717](file://options.js#L687-L717)
- [content.js:1419-1446](file://content.js#L1419-L1446)
- [content.js:1556-1570](file://content.js#L1556-L1570)

**章节来源**
- [options.html:646-682](file://options.html#L646-L682)
- [options.js:687-717](file://options.js#L687-L717)
- [content.js:1419-1446](file://content.js#L1419-L1446)
- [content.js:1556-1570](file://content.js#L1556-L1570)
- [_locales/en/messages.json:1-11](file://_locales/en/messages.json#L1-L11)
...
## 结论
本扩展通过清晰的模块划分与消息驱动的架构，实现了从设置到分析再到结果呈现的一体化体验。设置面板提供灵活的配置与历史管理，主面板提供直观的进度与复制能力，后台负责稳定高效的模型交互与错误处理。多语言支持贯穿设置与主面板，满足国际化需求。UI方面实现了全面的现代化改进，包括暗色主题、响应式设计、丰富的动画效果和自定义交互功能，显著提升了用户体验。新增的多语言支持增强功能进一步完善了国际化体验，包括设置面板的语言切换按钮和主面板的双语言显示模式，为全球用户提供更好的本地化服务。建议在生产环境中持续关注错误分类与用户提示的细化，以及历史记录容量与性能的平衡。
