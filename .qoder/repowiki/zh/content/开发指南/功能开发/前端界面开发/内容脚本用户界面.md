...
## 更新摘要
**变更内容**
- 增强了高保真重建模式支持：新增负向提示词显示、参数摘要展示、增强的用户反馈机制
- 扩展了结构化提示词处理：支持 negative 和 parameters 字段的完整显示和编辑
- 改进了 JSON 模式交互：增强了结构化数据的可视化和编辑能力
- 优化了用户反馈系统：提供更详细的生成状态和错误信息
...
- [options.html:1-722](file://options.html#L1-L722)
...
- **增强**：高保真重建模式
  - 支持负向提示词（negative）显示和编辑
  - 支持技术参数摘要（parameters）的结构化展示
  - 提供增强的用户反馈机制和状态指示
...
- **增强**：JSON模式支持
  ...
  - setTextareaValue()根据isJsonMode决定显示格式，支持负向提示词和参数摘要
...
#### 负向提示词显示
- **新增**：支持负向提示词（negative）字段的显示和编辑
- 在英文模式下，负向提示词会显示在生成内容下方，格式为 "Negative: [提示词]"
- 用户可以通过 JSON 模式直接编辑负向提示词内容
- 负向提示词用于防止生成漂移，确保精确重建

#### 参数摘要展示
- **新增**：支持技术参数摘要（parameters）字段的结构化展示
- 参数包括：subject_scale（主体比例）、layout_lock（布局锁定）、typography_lock（字体锁定）
- material_lock（材质锁定）、background_lock（背景锁定）、camera_lens（相机镜头）
- lighting_setup（灯光设置）、render_quality（渲染质量）等关键重建控制参数
- 参数以结构化 JSON 格式显示，便于用户理解和编辑

#### 增强的用户反馈机制
- **改进**：提供更详细的生成状态和进度反馈
- 进度文本包含耗时信息，格式为 "状态 · Xxs"
- 错误信息更加具体，包含错误类型和解决方案建议
- 成功状态下显示完成状态和复制按钮反馈

```mermaid
flowchart TD
Start(["高保真重建开始"]) --> CheckMode{"是否启用高保真模式?"}
CheckMode --> |是| EnableNeg["启用负向提示词"]
CheckMode --> |是| EnableParams["启用参数摘要"]
CheckMode --> |否| NormalMode["普通模式"]
EnableNeg --> EnableParams
EnableParams --> ProcessPrompts["处理结构化提示词"]
NormalMode --> ProcessPrompts
ProcessPrompts --> DisplayNeg["显示负向提示词"]
DisplayNeg --> DisplayParams["显示参数摘要"]
DisplayParams --> UpdateUI["更新界面显示"]
UpdateUI --> TrackEvent["记录用户交互事件"]
TrackEvent --> End(["完成"])
```

**图表来源**
- [content.js:58-92](file://content.js#L58-L92)
- [content.js:1643-1671](file://content.js#L1643-L1671)
...
- 切换要点
  ...
  - **增强**：JSON模式状态管理，统一按钮状态同步
  - **新增**：高保真模式状态管理，支持负向提示词和参数摘要显示
- 语言与文案
...
## 依赖关系分析
- content.js 依赖
  - config.js：共享配置与 UI 文案，包含高保真重建模式配置
...
## 结论
content.js 通过 Shadow DOM 实现 UI 隔离与主题适配，结合面板状态机与后台消息驱动，提供了稳定、可扩展的用户界面。本次重大改进包括高保真重建模式的增强支持，包括负向提示词显示、参数摘要展示、增强的用户反馈机制等，进一步提升了用户体验和系统的可靠性。遵循本文的扩展与优化建议，可在不破坏现有样式的前提下快速迭代新功能，并保持良好的性能与可维护性。
...
- 关键实现路径参考
  ...
  - [共享配置与多语言文案:4-321](file://config.js#L4-L321)
  - [设置页与存储同步:1-722](file://options.html#L1-L722), [options.js:1-551](file://options.js#L1-L551)
  ...
  - [高保真重建模式配置:17-21](file://config.js#L17-L21)
  - [负向提示词处理:58-92](file://content.js#L58-L92)
  - [参数摘要显示:1643-1671](file://content.js#L1643-L1671)
- **更新** 重大改进参考
  - [高保真重建模式增强:17-21](file://config.js#L17-L21)
  - [负向提示词显示:58-92](file://content.js#L58-L92)
  - [参数摘要展示:1643-1671](file://content.js#L1643-L1671)
  - [JSON模式与语言切换:1415-1461](file://content.js#L1415-L1461)
  - [增强的用户反馈机制:495-530](file://content.js#L495-L530)
