...
## 更新摘要
**变更内容**
- **功能状态更新**：根据当前实际代码，设置页面已完全实现，包含完整的配置管理、历史记录查看和交互体验
- **高保真重建模式**：已完全实现 recreateMode 设置，包括动态系统提示词构建
- **增强用户体验**：在 Experience 面板中添加了 recreateMode 开关
- **完善提示词构建**：后台脚本实现了智能的系统和用户提示词叠加逻辑
- **技术参数支持**：支持生成包含负面提示词和参数的技术字段
...
**重要说明** 本版本已完全实现所有功能，包括高保真重建模式、完整的预设芯片管理系统、历史记录管理和用户界面组件。与之前的"丢弃变更"描述不同，当前版本提供了完整的功能实现。

本指南将深入分析设置页面的 HTML 结构设计、JavaScript 逻辑实现，以及与后台脚本的通信机制，为开发者提供全面的开发参考。
...
### 高保真重建模式
- **recreateMode 设置**：新增的高精度重建模式开关
- **动态系统提示词**：根据模式自动构建详细的系统提示词
- **技术参数支持**：生成包含负面提示词和参数字段的提示词
- **智能叠加逻辑**：在现有提示词基础上添加详细重建约束
...
- [content.js:181-200](file://content.js#L181-L200)
...
#### 高保真重建模式界面
- **开关控件**：现代化的 switch 控件实现 recreateMode 切换
- **智能提示**：根据模式状态显示不同的说明文本
- **状态同步**：与后台脚本的设置状态保持同步
...
```mermaid
flowchart TD
RecreateMode[recreateMode 开关] --> CheckState{检查模式状态}
CheckState --> |启用| EnableMode[启用详细重建]
CheckState --> |禁用| DisableMode[使用标准模式]
EnableMode --> BuildSystemPrompt[构建详细系统提示词]
EnableMode --> BuildUserPrompt[构建详细用户提示词]
BuildSystemPrompt --> AddOverlay[添加重建覆盖层]
BuildUserPrompt --> AddOverlay
AddOverlay --> UpdatePayload[更新配置负载]
DisableMode --> UseStandard[使用标准提示词]
UpdatePayload --> SaveSettings[保存设置]
UseStandard --> SaveSettings
SaveSettings --> NotifyBackground[通知后台脚本]
```

**图表来源**
- [options.js:678](file://options.js#L678)
...
```javascript
// 高保真重建模式配置
const recreateModeConfig = {
    enabled: false,
    systemOverlay: "HIGH FIDELITY RECONSTRUCTION MODE:\n- This mode wraps around all existing instructions and adds extra detail requirements without replacing the base rules.\n- Ignore any earlier brevity or character-limit instruction. Produce exhaustive, reconstruction-oriented output.",
    userOverlay: "High fidelity reconstruction mode is enabled. Keep all existing instructions valid and add an outer layer of ultra-detailed reconstruction analysis. Ignore any earlier brevity or 750-character limit and be exhaustive. Capture tiny but reconstruction-critical details...",
    maxTokens: {
        openai: 2600,
        anthropic: 4000
    }
...
// 构建高保真重建提示词
function buildRecreateModePrompt(basePrompt, settings) {
    if (!settings.recreateMode) return basePrompt;
    
    const parts = [basePrompt];
    
    // 添加重建模式覆盖层
    if (settings.modelType === 'openai') {
        parts.push(config.RECREATE_MODE_USER_OVERLAY_OPENAI);
    } else {
        parts.push(config.RECREATE_MODE_USER_OVERLAY_ANTHROPIC);
    }
...
participant Storage as Chrome存储
participant Background as 后台脚本
Options->>Storage : 保存 recreateMode 设置
Options->>Background : 发送 settings : updated 消息
Background->>Background : 检查 recreateMode 状态
Background->>Background : 构建详细提示词
Background->>Background : 调整 token 限制
Background->>Content : 广播设置更新
Content->>Content : 更新生成逻辑
```
...
### 高保真重建模式性能
高保真重建模式的性能考虑：
- **token 限制调整**：根据模型类型调整最大 token 数量
- **智能提示词构建**：仅在启用模式时添加详细覆盖层
- **内存优化**：避免重复构建相同的提示词内容
...
#### 高保真重建模式异常
1. **检查模型兼容性**：确认所选模型支持详细重建模式
2. **验证 token 限制**：检查模型的最大 token 数量设置
3. **查看日志输出**：检查后台脚本是否有错误信息
...
**重要说明** 与之前的"丢弃变更"描述不同，当前版本已完全实现所有功能，包括：

- **高保真重建模式**：新增 recreateMode 设置，支持更精确的图像还原
- **智能提示词构建**：后台脚本实现了动态的系统和用户提示词叠加
- **技术参数支持**：生成包含负面提示词和参数字段的完整提示词
- **性能优化**：针对重建模式的 token 限制和内存使用进行了优化
...
1. **配置分离**：将重建模式的配置与普通配置分离
2. **智能叠加**：仅在启用模式时添加详细覆盖层
3. **模型适配**：根据不同模型类型调整 token 限制
4. **性能监控**：监控重建模式对性能的影响
5. **用户反馈**：提供清晰的模式状态指示
...
#### 模式配置
- **recreateMode 开关**：Experience 面板中的独立开关
- **动态系统提示词**：根据模式状态构建详细的系统提示词
- **用户提示词覆盖层**：添加重建相关的详细约束和要求

#### 提示词构建机制
- **系统提示词叠加**：在基础系统提示词上添加重建覆盖层
- **用户提示词增强**：添加高精度重建的技术要求
- **智能 token 管理**：根据模型类型调整最大 token 数量

#### 技术参数生成
- **负面提示词**：生成防止漂移的负面提示词
- **参数字段**：包含主题比例、布局锁定、材质锁定等参数
- **结构化输出**：确保生成的参数字段格式正确
...
- [options.js:669-685](file://options.js#L669-L685)
- [options.js:27-82](file://options.js#L27-L82)
- [options.js:153-171](file://options.js#L153-L171)
- [background.js:639-649](file://background.js#L639-L649)
